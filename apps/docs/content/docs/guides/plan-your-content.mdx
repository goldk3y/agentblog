...
If you have a [DataForSEO](https://dataforseo.com) account, run the
`dataforseo-research` skill before this step and plan from measurement instead.
It reports what you already rank for, which of your posts sits at positions 4 to
20 and needs an edit rather than a replacement, who actually holds the queries
you want, and whether AI engines cite you. `plan-blog-content` reads its output
from `content/research/` and only falls back to the method below when there is
nothing there. See [write posts with your coding
agent](/guides/write-with-your-agent#research-and-the-one-skill-that-needs-an-account).

Without that, you already have a supply of questions:
