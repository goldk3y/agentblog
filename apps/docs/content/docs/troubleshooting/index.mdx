...
Exclude only what you are certain the route does not read. The MDX source
genuinely does read `content/blog/**` at runtime when the publish webhook
regenerates the sitemap and the feed, so that directory has to stay.
