...
You have a `src/` layout. `agentblog.config.ts` belongs at the project root,
next to `next.config.ts`, while `@/` points at `src/`, so the one import in
`lib/config.ts` resolves to nothing and the build stops at module resolution.

```bash
npx agentblog@latest doctor --fix
```

That writes one entry in `tsconfig.json`. You can also add it by hand:
...
Do not move `agentblog.config.ts` into `src/` instead. `agentblog doctor`, the
`~/` target in the registry, and every other config file in a Next.js project all
expect it at the root, so moving it trades one broken import for a file nothing
else can find.
