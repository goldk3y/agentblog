...
| `@agentblog/agent-kit`   | The seven agent skills on their own            |
...
`agentblog.dev` serves the registry, so you do not need this to install. It is
for working against a fork or a change that has not shipped yet: you point your
project at a registry running on your own machine, which is fine for a scratch
app and not for anything you deploy.
